...
def play_pisti():
    ...
    for i in range(1, 7):
        ...
        for j in range(1, 5):
            ...
            while 1:
                ...
                move = str(input('Your Move')).upper()

                if move in PlayerHand:

                    if len(table) > 0:  # one more control due to jack card

                        if move[0] == 'J':  # if card is jack
                            ...
                            if move[0] == table[-1][0]:  # if cards are same with table
            ...
            print(f"Player Point:{plp} //// AI Point: {aip}")
        if i == 6:
    ...
    return int(input('Would you like to play again?(1 to play again):'))
